...
case "$PROFILE" in
	core) PY_LOCK="requirements-core-lock-linux.txt" ;;
	full) PY_LOCK="requirements-lock-linux.txt" ;;
	*)
		echo "Usage: ./install.sh [core|full]" >&2
		exit 2
		;;
esac
...
"$PYTHON" -m pip install -r "$ROOT/python/$PY_LOCK"
"$PYTHON" -m pip check
"$PYTHON" "$ROOT/python/scripts/check_installed_lock.py" --lock "$PY_LOCK"
