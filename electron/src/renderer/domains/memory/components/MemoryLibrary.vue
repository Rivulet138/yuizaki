<template>
  <section class="library" aria-labelledby="memory-library-title">
    <div class="library-heading">
      <div><h3 id="memory-library-title">记忆文档</h3><p role="status" aria-live="polite">匹配 {{ filteredCount }} 条 · 已加载 {{ visibleDocs.length }} 条</p></div>
      ...
      <div class="master-detail">
        <div class="memory-list" role="listbox" aria-label="记忆列表">
          <button v-for="doc in visibleDocs" :key="doc.id" type="button" role="option" class="memory-row" :aria-selected="selectedDoc?.id === doc.id" :class="{ hit: isQueryHit(doc) }" :data-memory-id="doc.id" @click="emit('select', doc)">
            <span class="doc-title"><strong>{{ doc.type || '记忆' }}</strong><el-tag size="small" :type="layerTagType(doc.layer)">{{ doc.layer || '未分类' }}</el-tag></span>
            <p>{{ doc.text }}</p>
            <span class="doc-meta"><span>重要度 {{ Number(doc.importance ?? 0).toFixed(2) }}</span><span>质量 {{ formatScore(doc.quality_score) }}</span><span>{{ docUpdatedLabel(doc) }}</span></span>
          </button>
        ...
        <MemoryInspector
          ...
          :metadata-preview="metadataPreview" :audit-entries="docAuditEntries" :audit-action-label="auditActionLabel" :audit-entry-summary="auditEntrySummary"
