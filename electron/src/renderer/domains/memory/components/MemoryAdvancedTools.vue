<template>
  <div class="advanced-tools">
    <section>
      ...
      <el-button plain :loading="rebuildIndexLoading" @click="emit('rebuild-index')">重建索引</el-button>
    </section>

    <section>
      <div class="section-head"><div><h3>原始检索测试</h3><p>输入查询并查看召回结果与分数。</p></div><el-tag v-if="querySummary" type="success">{{ querySummary }}</el-tag></div>
      ...
      <div v-if="queryTrace" class="trace" role="status"><strong>检索轨迹</strong><span>召回 {{ queryTrace.recall_count ?? queryResult?.results?.length ?? 0 }} · 候选 {{ queryTrace.candidate_count ?? 0 }} · 过滤 {{ queryTrace.filtered_out_count ?? 0 }} · {{ formatLatency(queryTrace.latency_ms) }}</span><span v-if="filterReasonText">{{ filterReasonText }}</span><code v-if="selectedTraceIds.length">{{ selectedTraceIds.join(' · ') }}<template v-if="hiddenTraceIdCount"> +{{ hiddenTraceIdCount }}</template></code></div>
    <AsyncState :loading="queryLoading" :error="queryError" :empty="!queryResult?.results?.length" empty-text="暂无检索结果" @retry="emit('query')"><div class="results" data-testid="memory-query-results"><article v-for="(row,index) in queryResult?.results || []" :key="row.id || index" :data-memory-query-id="row.id || undefined"><b>{{ index + 1 }}</b><div><p>{{ row.text }}</p><span>得分 {{ Number(row.score ?? 0).toFixed(4) }}</span><div class="score-components" aria-label="召回分数构成"><span v-for="component in scoreComponentRows(row.score_components, row.score)" :key="component.key"><small>{{ component.label }}</small><strong>{{ component.value }}</strong></span></div><el-button v-if="row.id" link type="primary" @click="emit('select-result', row.id)">查看记忆</el-button></div></article></div></AsyncState>
    </section>
...
<el-form label-position="top" @submit.prevent="emit('write-document')"><el-form-item label="文档 ID（可选）"><el-input :model-value="documentForm.id" @update:model-value="updateDocument('id', String($event))" /></el-form-item><el-form-item label="文档内容"><el-input :model-value="documentForm.text" data-testid="memory-document-text" type="textarea" :rows="4" @update:model-value="updateDocument('text', String($event))" /></el-form-item><el-form-item label="元数据 JSON"><el-input :model-value="documentForm.metadataJson" data-testid="memory-document-metadata" type="textarea" :rows="3" @update:model-value="updateDocument('metadataJson', String($event))" /></el-form-item><el-button data-testid="memory-document-submit" type="primary" plain native-type="submit" :loading="documentLoading" :disabled="!documentForm.text.trim()">写入原始文档</el-button></el-form>
    ...
    <section>
      <div class="section-head"><div><h3>永久清理</h3><p>先预览影响，再从本机存储永久删除。</p></div><el-tag data-testid="memory-maintenance-summary" :type="maintenancePreview?.summary.delete_count ? 'danger' : 'info'">{{ maintenancePreview ? `${maintenancePreview.summary.delete_count} 条待清理` : '尚未预览' }}</el-tag></div>
...
defineProps<{ indexStatus:MemoryIndexStatus|null; indexStatusLabel:string; indexAvailabilityLabel:string; indexStatusTone:TagType; docCount:number; rebuildIndexLoading:boolean; queryForm:MemoryQueryForm; layers:MemoryLayer[]; effectiveQueryLayers:string[]; queryLoading:boolean; rawQueryLoading:boolean; queryError?:string; queryResult:any; queryTrace:any; querySummary:string; filterReasonText:string; selectedTraceIds:string[]; hiddenTraceIdCount:number; documentForm:MemoryDocumentForm; documentLoading:boolean; maintenancePolicy:MemoryMaintenancePolicy; maintenancePreview:MemoryMaintenancePreview|null; maintenancePreviewMatchesPolicy:boolean; maintenanceSaving:boolean; maintenancePreviewLoading:boolean; maintenanceApplyLoading:boolean; formatLatency:(value?:number|null)=>string; compactText:(text?:string|null,limit?:number)=>string; maintenanceReasonLabel:(reasons:string[])=>string }>()
const emit=defineEmits<{ 'rebuild-index':[];query:[];'raw-query':[];'toggle-query-layer':[layer:string];'reset-query-layers':[];'select-result':[id:string];'write-document':[];'save-maintenance':[];'preview-maintenance':[];'apply-maintenance':[];'update-query':[value:Partial<MemoryQueryForm>];'update-document':[value:Partial<MemoryDocumentForm>];'update-maintenance':[value:Partial<MemoryMaintenancePolicy>] }>()
