<template>
  <div class="advanced-tools">
    <section>
      <div class="section-head"><div><h3>搜索索引</h3></div><el-tag :type="indexStatusTone">{{ indexStatusLabel }}</el-tag></div>
    ...
    <section>
      <div class="section-head"><div><h3>召回检索</h3></div><el-tag v-if="querySummary" type="success">{{ querySummary }}</el-tag></div>
      <el-form label-position="top" @submit.prevent="emit('query')">
        ...
        <div class="button-row"><el-button data-testid="memory-query-submit" type="primary" native-type="submit" :loading="queryLoading" :disabled="!queryForm.query.trim()">执行检索</el-button><el-button link type="primary" @click="emit('reset-query-layers')">恢复默认层级</el-button></div>
      </el-form>
      <div v-if="queryTrace" class="trace" role="status"><strong>检索轨迹</strong><span>召回 {{ queryTrace.recall_count ?? queryResult?.results?.length ?? 0 }} · 候选 {{ queryTrace.candidate_count ?? 0 }} · 过滤 {{ queryTrace.filtered_out_count ?? 0 }} · {{ formatLatency(queryTrace.latency_ms) }}</span><span>锚点 {{ queryTrace.anchor_ids?.length ?? 0 }} · 关联 {{ queryTrace.expanded_ids?.length ?? 0 }} · 关联耗时 {{ formatLatency(queryTrace.relation_latency_ms) }}<template v-if="queryTrace.expansion_truncated"> · 已达到扩展上限</template></span><span>证据覆盖率 {{ Math.round((queryTrace.evidence_coverage ?? 0) * 100) }}% · 关联证据估算 {{ queryTrace.relation_token_estimate ?? 0 }} tokens</span><span v-if="queryTrace.context_budget_tokens">上下文预算 {{ queryTrace.context_token_estimate ?? 0 }}/{{ queryTrace.context_budget_tokens }} tokens<template v-if="queryTrace.budget_truncated"> · 已截断</template></span><span v-if="queryTrace.index_consistency && queryTrace.index_consistency !== 'current'">索引状态：{{ queryTrace.index_consistency }}</span><span v-if="filterReasonText">{{ filterReasonText }}</span><code v-if="selectedTraceIds.length">{{ selectedTraceIds.join(' · ') }}<template v-if="hiddenTraceIdCount"> +{{ hiddenTraceIdCount }}</template></code></div>
      <AsyncState :loading="queryLoading" :error="queryError" :empty="!queryResult?.results?.length" empty-text="暂无检索结果" :show-retry="false"><div class="results" data-testid="memory-query-results"><article v-for="(row,index) in queryResult?.results || []" :key="row.id || index" :data-memory-query-id="row.id || undefined"><b>{{ index + 1 }}</b><div><p>{{ row.text }}</p><span>得分 {{ Number(row.score ?? 0).toFixed(4) }}</span><div v-if="row.why_recalled || row.evidence_type" class="recall-explanation"><strong>{{ row.why_recalled || '按相关性排序进入结果' }}</strong><small v-if="row.evidence_type">{{ evidenceTypeLabel(row.evidence_type) }}<template v-if="row.association"> · {{ row.association }}</template><template v-if="helpfulRecallCount(row)"> · 已帮助 {{ helpfulRecallCount(row) }} 次</template></small></div><div class="score-components" aria-label="召回分数构成"><span v-for="component in scoreComponentRows(row.score_components, row.score)" :key="component.key"><small>{{ component.label }}</small><strong>{{ component.value }}</strong></span></div><div v-if="row.id" class="result-actions"><el-button link type="primary" @click="emit('select-result', row.id)">查看记忆</el-button><div class="feedback-row" role="group" aria-label="召回反馈"><el-button size="small" plain :type="feedbackFor(row.id) === 'helpful' ? 'success' : undefined" @click="sendFeedback(row.id, 'helpful')">有帮助</el-button><el-button size="small" plain :type="feedbackFor(row.id) === 'not_helpful' ? 'warning' : undefined" @click="sendFeedback(row.id, 'not_helpful')">没帮助</el-button><el-button size="small" plain :type="feedbackFor(row.id) === 'incorrect' ? 'danger' : undefined" @click="sendFeedback(row.id, 'incorrect')">不准确</el-button><el-button size="small" link :type="feedbackFor(row.id) === 'dismissed' ? 'info' : undefined" @click="sendFeedback(row.id, 'dismissed')">暂不评价</el-button></div></div></div></article></div></AsyncState>
    </section>

    <details class="maintenance-section">
      <summary><span><strong>维护清理</strong></span><el-tag data-testid="memory-maintenance-summary" :type="maintenancePreview?.summary.delete_count ? 'danger' : 'info'">{{ maintenancePreview ? `${maintenancePreview.summary.delete_count} 条待清理` : '未预览' }}</el-tag></summary>
      <section>
...
import type { MemoryQueryResult, MemoryRetrievalTrace } from '../composables/useMemoryDomain'
const props=defineProps<{ indexStatus:MemoryIndexStatus|null; rebuildJob:MemoryIndexRebuildJob|null; indexStatusLabel:string; indexAvailabilityLabel:string; indexStatusTone:TagType; docCount:number; rebuildIndexLoading:boolean; queryForm:MemoryQueryForm; layers:MemoryLayer[]; effectiveQueryLayers:string[]; queryLoading:boolean; queryError?:string; queryResult:MemoryQueryResult|null; querySummary:string; filterReasonText:string; maintenancePolicy:MemoryMaintenancePolicy; maintenancePreview:MemoryMaintenancePreview|null; maintenancePreviewMatchesPolicy:boolean; maintenanceSaving:boolean; maintenancePreviewLoading:boolean; maintenanceApplyLoading:boolean; formatLatency:(value?:number|null)=>string; compactText:(text?:string|null,limit?:number)=>string; maintenanceReasonLabel:(reasons:string[])=>string }>()
const emit=defineEmits<{ 'rebuild-index':[];'cancel-rebuild-index':[];query:[];'toggle-query-layer':[layer:string];'reset-query-layers':[];'select-result':[id:string];feedback:[payload:{id:string;feedback:MemoryRecallFeedback}];'save-maintenance':[];'preview-maintenance':[];'apply-maintenance':[];'update-query':[value:Partial<MemoryQueryForm>];'update-maintenance':[value:Partial<MemoryMaintenancePolicy>] }>()
const queryTrace = computed<MemoryRetrievalTrace | null>(() => props.queryResult?.trace ?? null)
...
.maintenance-section{padding-bottom:22px;border-bottom:1px solid var(--yui-border)}.maintenance-section>summary{display:flex;align-items:center;justify-content:space-between;gap:12px;color:var(--yui-text);cursor:pointer;list-style:none}.maintenance-section>summary::-webkit-details-marker{display:none}.maintenance-section>summary>span{display:flex;min-width:0;flex-direction:column;gap:3px}.maintenance-section>summary strong{font-size:15px}.maintenance-section>summary small{color:var(--yui-muted);font-size:12px}.maintenance-section section{padding:14px 0 0;border-bottom:0}
