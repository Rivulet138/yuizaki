...
describe('AgentTracePanel tool jobs', () => {
  it('offers request-scoped cancellation and correlated retry for tool jobs', () => {
    ...
    expect(panel).toContain("executing: '执行中'")
    expect(panel).toContain("verified: '已验证'")
    expect(panel).toContain("failed: '失败'")
    expect(panel).toContain("unknown_effect: '结果未知'")
    expect(panel).toContain('companionJobEvidence(job)')
  })
})
